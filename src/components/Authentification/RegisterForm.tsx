...
const FIELDS = [
  { id: "username",        label: "Nom d'utilisateur", icon: <User className="w-4 h-4" />, type: "text",     placeholder: "nom",           autoComplete: "username" },
  { id: "email",           label: "Adresse email",     icon: <Mail className="w-4 h-4" />, type: "email",    placeholder: "astronaute@univers.com", autoComplete: "email" },
  { id: "password",        label: "Mot de passe",      icon: <Lock className="w-4 h-4" />, type: "password", placeholder: "••••••••",               autoComplete: "new-password" },
  { id: "confirmPassword", label: "Confirmer",         icon: <Lock className="w-4 h-4" />, type: "password", placeholder: "••••••••",               autoComplete: "new-password" },
] as const;
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    const data = await api.post<{ token: string; user: { id: number; name: string; email: string; role: "rédacteur" | "administrateur"; } }>("/auth/login", {
      email: form.email,
      password: form.password,
    });
  ...
  bg-gradient-to-br from-[#020617] via-[#0F172A] to-[#020617]">
    ...
    <div className="w-full max-w-md bg-white/[0.04] backdrop-blur-2xl border border-white/10 rounded-2xl p-8 shadow-2xl z-10">
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-5">
        {errors.general && (
          <p className="flex items-center gap-2 text-sm text-red-300 bg-red-500/10 border border-red-500/30 rounded-xl px-4 py-3" role="alert">
