...
export default function DetailsSpaceArticle() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    api
      .get<SpaceArticle>(`/api/spaceflight/articles/${id}`)
      .then((art) => {
        setArticle(art);
        return api
          .get<{ results: SpaceArticle[] }>("/api/spaceflight/articles?limit=6")
          .then((res) =>
            setRelated(res.results.filter((a) => a.id !== art.id).slice(0, 3)),
          );
      })
      .catch(() => setError(true))
        ...
        <div className="border-t border-slate-800 py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((a) => (
                <SpaceArticleCard key={a.id} article={toArticle(a)} />
              ))}
            </div>
          </div>
        </div>
