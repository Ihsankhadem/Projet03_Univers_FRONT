...
export default function DetailsSpaceArticle() {
  ...
  useEffect(() => {
    ...
    api.get<SpaceArticle>(`/api/spaceflight/articles/${id}`)
      .then((art) => {
        setArticle(art);
        return api.get<{ results: SpaceArticle[] }>("/api/spaceflight/articles?limit=6")
          .then((res) => setRelated(res.results.filter((a) => a.id !== art.id).slice(0, 3)));
      })
      .catch(() => setError(true))
  ...
  if (error || !article) return (
    <div className="min-h-screen bg-[#0B0F1A] flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/articles" className="text-violet-400 hover:underline text-sm">← Retour</Link>
    </div>
  );
      ...
      {related.length > 0 && (
        <div className="border-t border-slate-800 py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <p className="text-blue-400 uppercase tracking-widest text-xs font-semibold mb-2">À lire aussi</p>
            <h2 className="text-2xl font-bold text-slate-100 mb-8">Actualités similaires</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((a) => <SpaceArticleCard key={a.id} article={toArticle(a)} />)}
            </div>
          </div>
        </div>
      )}
